...
def lstm_pipeline(models):
    ...
    for model in models:
        ...
        for data_point in model.data:
            ...
            data_formatted = np.concatenate((data_formatted, np.zeros((model.max_seq_lens[data_type] - data_formatted.shape[0], data_formatted.shape[1]))), axis = 0)
    ...
    #window = 100
    #for student in students:
    #    if (len(data_by_student[student]) < window):
    #        window = len(data_by_student[student])
    #window -= 1
    window = 19
    ...
    for student in students:
        for i in range(len(data_by_student[student]) - window -1):
            data_formatted = np.concatenate(data_by_student[student][i:i+window], axis=0)
            #print(data_formatted.shape)
            X.append(np.concatenate(data_by_student[student][i:i+window], axis=0))
...
def lstm_predict(X_train, X_test, y_train, y_test):
    #print("X_train:")
    #print(type(X_train))
    #print("X_train at index 0: ")
    #print(X_train[0])
    num_timesteps = X_train[0].shape[0]
...
def type_student_pipeline(models):
    data_types = ['post', 'click', 'event']
    students = [model.name for model in models]
    data_by_student = dict()
    y_by_student = dict()
